...
export function proxy(req){
 ...
 if(process.env.INGEST_TOKEN&&['/api/items','/api/admin/seed','/api/admin/seed/app/api/admin/seed'].includes(req.nextUrl.pathname)&&req.method==='POST'&&same(auth,`Bearer ${process.env.INGEST_TOKEN}`))return NextResponse.next();
